...
std::shared_ptr<Resource> CreateSRVFromFileHDR(RenderDevice& device, RenderCommandList& command_list, const std::string& path)
{
    int width = 0;
    int height = 0;
    int channels = 0;
    const float* image = stbi_loadf(path.c_str(), &width, &height, &channels, SOIL_LOAD_RGB);
    if (!image)
        return {};

    gli::format format = gli::format::FORMAT_RGB32_SFLOAT_PACK32;
    std::shared_ptr<Resource> res = device.CreateTexture(BindFlag::kShaderResource | BindFlag::kCopyDest, format, 1, width, height);
...
std::shared_ptr<Resource> CreateTexture(RenderDevice& device, RenderCommandList& command_list, const std::string& path)
{
    ...
    else if(path.find(".hdr") != -1)
        return CreateSRVFromFileHDR(device, command_list, path);
    else
