...
int main(int argc, char* argv[])
{
    ...
    AppBox app("10 - DrawIndirect", settings);
...
//    upload_command_list->UpdateSubresource(count_buffer, 0, &count, 0, 0);
    ...
    for (uint32_t i = 0; i < settings.frame_count; ++i)
    {
        ...
        {
            command_list->BeginEvent("Task Pass");
            command_list->UseProgram(taskProgram);
            command_list->Attach(taskProgram.cs.uav.Argument, argument_buffer);
            command_list->Dispatch(1, 1, 1);
            command_list->EndEvent();
        }
        {
            command_list->BeginEvent("DrawIndirect Pass");
