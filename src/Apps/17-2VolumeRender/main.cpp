...
#include "ProgramRef/Wireframe_PS.h"
#include "ProgramRef/Wireframe_VS.h"
...
int main(int argc, char* argv[])
{
    ...
    AppBox app("17.2 - Volume Render", settings);
...
//    std::shared_ptr<Resource> volumeTextureSlice = CreateTexture(*device, *upload_command_list, ASSETS_PATH"textures/volume/volume_pig_y.png");
    ...
    ProgramHolder<Wireframe_PS, Wireframe_VS> progromWireFrame(*device);
    ...
    bool bDrawDebugWireframe = false;
    ImVec4 wireframe_color = ImVec4(0.7f, 0.0f, 0.60f, 1.00f);
    ...
    while (!app.PollEvents())
    {
...

        {
            ...
            {
                // Debug wireframe
                if (bDrawDebugWireframe)
                {
                    progromWireFrame.ps.cbuffer.settings.wireColor = {wireframe_color.x, wireframe_color.y, wireframe_color.z, wireframe_color.w};

                    progromWireFrame.vs.cbuffer.Constants.World = glm::transpose(model.matrix);
                    progromWireFrame.vs.cbuffer.Constants.WorldView = progromWireFrame.vs.cbuffer.Constants.World * glm::transpose(camera.GetViewMatrix());
                    progromWireFrame.vs.cbuffer.Constants.WorldViewProj = progromWireFrame.vs.cbuffer.Constants.WorldView * glm::transpose(camera.GetProjectionMatrix());
                    progromWireFrame.vs.cbuffer.Constants.CameraPos = camera.position_;

                    command_lists[frameIndex]->UseProgram(progromWireFrame);
                    command_lists[frameIndex]->SetRasterizeState({ FillMode::kWireframe, CullMode::kNone });
//                    command_lists[frameIndex]->SetRasterizeState({ FillMode::kSolid, CullMode::kNone });
                    command_lists[frameIndex]->SetDepthStencilState({ false });
                    command_lists[frameIndex]->Attach(progromWireFrame.ps.cbv.settings, progromWireFrame.ps.cbuffer.settings);
                    command_lists[frameIndex]->Attach(progromWireFrame.vs.cbv.Constants, progromWireFrame.vs.cbuffer.Constants);
                    for (auto& range : model.ia.ranges)
                    {
                        model.ia.indices.Bind(*command_lists[frameIndex]);
                        model.ia.positions.BindToSlot(*command_lists[frameIndex], progromWireFrame.vs.ia.POSITION);
                        model.ia.texcoords.BindToSlot(*command_lists[frameIndex], progromWireFrame.vs.ia.TEXCOORD);
                        command_lists[frameIndex]->DrawIndexed(range.index_count, 1, range.start_index_location, range.base_vertex_location, 0);
                    }
                }
            }
...

        {
...

            {
                ...
                ImGui::Checkbox("Draw Debug Wireframe", &bDrawDebugWireframe);
                ImGui::ColorEdit4("clear color", (float*)&wireframe_color);
                ImGui::SliderFloat("alpha", &alpha, 0.0f, 1.0f, "alpha = %.3f");
                ImGui::SliderFloat("setpsize", &stepSize, 0.0f, 0.01f, "setpsize = %.4f");
                ImGui::SliderInt("maxStepCount", &maxStepCount, 1, 1024);
